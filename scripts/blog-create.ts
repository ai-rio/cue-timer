...
const TEMPLATE_DESCRIPTIONS: Record<string, string> = {
  'timing-guide': 'Step-by-step guides for mastering presentation timing techniques',
  'case-study': 'Success stories and real-world applications of CueTimer',
  'feature-announce': 'Announce new features and updates to the CueTimer platform',
  'presentation-tips': 'Practical tips and best practices for better presentations',
};
...
async function collectTimingGuideVariables(cliOptions: CliOptions): Promise<TimingGuideVariables> {
      ...
      const stepAnswers = await inquirer.prompt([
        {
          ...
          validate: (input: string) => input.trim().length > 0 || 'Step title is required',
        },
        {
          type: 'input',
          name: 'description',
          message: `Step ${stepIndex} description:`,
          validate: (input: string) => input.trim().length > 0 || 'Step description is required',
        },
        {
          type: 'input',
          name: 'time',
          ...
          const tipAnswer = await inquirer.prompt([
            {
              ...
              validate: (input: string) => input.trim().length > 0 || 'Tip cannot be empty',
            },
            {
              type: 'confirm',
              name: 'addMore',
...
async function collectCaseStudyVariables(cliOptions: CliOptions): Promise<CaseStudyVariables> {
  const answers = await inquirer.prompt([
    {
      type: 'input',
      name: 'title',
    ...
    },
    {
      ...
      validate: (input: string) => input.trim().length > 0 || 'Client name is required',
    },
    {
      type: 'input',
      name: 'industry',
      message: 'Industry sector:',
      validate: (input: string) => input.trim().length > 0 || 'Industry is required',
    },
    {
      type: 'input',
      name: 'challenge',
      message: 'What was the main challenge faced?',
      validate: (input: string) => input.trim().length > 0 || 'Challenge is required',
    },
    {
      type: 'input',
      name: 'solution',
      message: 'How did CueTimer solve the challenge?',
      validate: (input: string) => input.trim().length > 0 || 'Solution is required',
    },
    {
      type: 'confirm',
      name: 'addResults',
      ...
      const resultAnswers = await inquirer.prompt([
        {
          ...
          validate: (input: string) => input.trim().length > 0 || 'Metric name is required',
        },
        {
          type: 'input',
          name: 'value',
      ...
      const quoteAnswers = await inquirer.prompt([
        {
          ...
          validate: (input: string) => input.trim().length > 0 || 'Quote text is required',
        },
        {
          type: 'input',
          name: 'author',
          message: `Quote ${quoteIndex} author:`,
          validate: (input: string) => input.trim().length > 0 || 'Author is required',
        },
        {
          type: 'input',
          name: 'role',
      ...
      const benefitAnswer = await inquirer.prompt([
        {
          ...
          validate: (input: string) => input.trim().length > 0 || 'Benefit cannot be empty',
        },
        {
          type: 'confirm',
          name: 'addMore',
      ...
      const useCaseAnswer = await inquirer.prompt([
        {
          ...
          validate: (input: string) => input.trim().length > 0 || 'Use case cannot be empty',
        },
        {
          type: 'confirm',
          name: 'addMore',
      ...
      const tipAnswers = await inquirer.prompt([
        {
          ...
          validate: (input: string) => input.trim().length > 0 || 'Tip title is required',
        },
        {
          type: 'input',
          name: 'description',
          message: `Tip ${tipIndex} description:`,
          validate: (input: string) => input.trim().length > 0 || 'Tip description is required',
        },
        {
          type: 'list',
          name: 'category',
...
async function createBlogPost(options: CliOptions): Promise<void> {
  try {
    ...
    if (options.interactive !== false) {
      console.log(chalk.gray("Interactive mode enabled. Let's create your blog post!\n"));
      ...
      const templateAnswer = await inquirer.prompt([
        {
          ...
          choices: Object.entries(TEMPLATE_DESCRIPTIONS).map(([key, description]) => ({
      ...
      const templateVariables = await collectTemplateVariables(template, options);
      ...
      console.log(
        chalk.white(`  Language: ${SUPPORTED_LANGUAGES.find((l) => l.value === language)?.name}`)
      );
      console.log(chalk.white(`  Author: ${author}`));
      console.log(chalk.white(`  Status: ${options.draft === false ? 'Published' : 'Draft'}`));
      ...
      try {
        ...
        console.log(chalk.white(`  Status: ${blogPost.isDraft ? 'Draft' : 'Published'}`));
        ...
        console.log(chalk.white('  3. Test the post in your development environment'));
...
program
  ...
  .option('--template <template>', `Template type (${Object.keys(TEMPLATES).join(', ')})`)
