...
async function findPostBySlug(slug: string): Promise<{ filePath: string; post: BlogPost } | null> {
  try {
    const years = await fs.readdir(CONTENT_DIR);
    ...
    for (const year of years) {
      ...
      for (const item of yearItems) {
        ...
        if (stat.isDirectory()) {
          ...
          for (const file of files) {
            if (file.endsWith('.mdx') || file.endsWith('.md')) {
              ...
              if (frontmatter.slug === slug) {
                return { filePath, post: { ...frontmatter, content: contentBody } };
              }
            }
          }
        } else if (stat.isFile() && (item.endsWith('.mdx') || item.endsWith('.md'))) {
          ...
          if (frontmatter.slug === slug) {
            return { filePath: itemPath, post: { ...frontmatter, content: contentBody } };
          }
        }
      }
    }
  ...
  titleWords.forEach((word) => {
    if (wordFreq[word]) {
      ...
      keywords.push({
        ...
        competition: density > 0.05 ? 'high' : density > 0.02 ? 'medium' : 'low',
      });
    }
  });
...
function checkDescriptionSEO(post: BlogPost): SEOIssue[] {
  ...
  if (post.summary.length < 120) {
    issues.push({
      type: 'suboptimal',
      field: 'summary',
      message: 'Summary is too short for optimal SEO (recommended: 120-160 characters)',
      severity: 'warning',
    });
  }
  ...
  if (post.summary.length > 160) {
    issues.push({
      type: 'suboptimal',
      field: 'summary',
      message: 'Summary is too long for optimal SEO (recommended: 120-160 characters)',
      severity: 'warning',
    });
  }
  ...
  if (
    post.title &&
    post.summary &&
    !post.summary.toLowerCase().includes(post.title.toLowerCase().split(' ')[0] || '')
  ) {
    issues.push({
      type: 'suboptimal',
      field: 'summary',
...
function checkContentSEO(post: BlogPost): SEOIssue[] {
  ...
  if (wordCount < 300) {
    issues.push({
      type: 'suboptimal',
      field: 'content',
      message: 'Content is too short for optimal SEO (recommended: minimum 300 words)',
      severity: 'warning',
    });
  }
  ...
  if (!post.slug) {
    ...
    if (post.slug.length > 60) {
      issues.push({
        type: 'suboptimal',
        field: 'slug',
        message: 'URL slug is too long for optimal SEO (recommended: max 60 characters)',
        severity: 'warning',
      });
    }
    ...
    if (!/^[a-z0-9-]+$/.test(post.slug)) {
      issues.push({
        type: 'invalid',
        field: 'slug',
        message: 'URL slug should contain only lowercase letters, numbers, and hyphens',
        severity: 'warning',
      });
    }
  }
...
function generateRecommendations(
  ...
  if (descriptionIssues.length > 0) {
    recommendations.push({
      category: 'content',
      priority: 'high',
      action: 'Write compelling meta description (120-160 characters) with target keywords',
  ...
  const keywordIssues = keywords.filter(
    (k) => k.density < KEYWORD_DENSITY_RANGE.min || k.density > KEYWORD_DENSITY_RANGE.max
  );
  ...
  const technicalIssues = issues.filter((i) => i.field === 'tags' || i.field === 'slug');
...
async function autoFixSeoIssues(postData: { filePath: string; post: BlogPost }): Promise<void> {
  ...
  if (!post.readTime || post.readTime === 0) {
    ...
    updatedContent = updatedContent.replace(/(readTime:\s*)\d+/, `$1${estimatedReadTime}`);
  ...
  if (!post.tags || post.tags.length === 0) {
    ...
    const suggestedTags = titleWords.filter((word) => word.length > 4).slice(0, 3);
  ...
  try {
    ...
    try {
      ...
      if (results.length === 1) {
        ...
        // Score display
        const scoreColor =
          seoResult.score >= 80 ? chalk.green : seoResult.score >= 60 ? chalk.yellow : chalk.red;
        console.log(chalk.bold('SEO Score: ') + scoreColor(`${seoResult.score}/100`));
        ...
        if (seoResult.issues.length > 0) {
          ...
          seoResult.issues.forEach((issue) => {
            ...
            console.log(chalk.red(`  ${icon} ${issue.field}: ${issue.message}`));
          });
        }
            ...
            const priorityColor =
              rec.priority === 'high'
                ...
                : rec.priority === 'medium'
                  ...
                  : chalk.green;
            console.log(
              chalk.white(`  ${priorityColor(`[${rec.priority.toUpperCase()}]`)} ${rec.action}`)
            );
        ...
        // Overall statistics
        const avgScore = results.reduce((sum, r) => sum + r.seoResult.score, 0) / results.length;
        const totalErrors = results.reduce(
          (sum, r) => sum + r.seoResult.issues.filter((i) => i.severity === 'error').length,
          0
        );
        const totalWarnings = results.reduce(
          (sum, r) => sum + r.seoResult.issues.filter((i) => i.severity === 'warning').length,
          0
        );

        console.log(chalk.blue.bold('\n📈 Overall Statistics:'));
        console.log(chalk.white(`  Average SEO Score: ${avgScore.toFixed(1)}/100`));
      ...
      if (options.fix && results.length === 1) {
        ...
        if (answer.proceed) {
          ...
          try {
            ...
            fixSpinner.succeed(chalk.green('SEO issues auto-fixed successfully!'));
...
program
  ...
  .action(() => {
    ...
    console.log(chalk.gray('  bun run scripts/blog-seo-check.ts --slug my-post-slug\n'));
    ...
    console.log(chalk.gray('  bun run scripts/blog-seo-check.ts --slug my-post-slug --fix\n'));

    console.log(chalk.green('Show SEO scores only:'));
    console.log(chalk.gray('  bun run scripts/blog-seo-check.ts --all --score\n'));
