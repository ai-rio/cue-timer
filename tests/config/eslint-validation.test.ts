...
describe('ESLint Configuration Validation', () => {
  ...
  describe('Configuration File Validation', () => {
    ...
    test('should have separate rules for different file types', () => {
      ...
      expect(configContent).toContain("files: ['content/**/*.md', 'content/**/*.mdx']");
    });
  });
  ...
  describe('File-Specific Rules', () => {
    test('should allow console in scripts', () => {
      ...
      expect(() => {
        execSync(`echo "${testFile}" | bunx eslint --stdin --ext .ts --config-filename scripts/`, {
          cwd: projectRoot,
          stdio: 'pipe',
        });
      }).not.toThrow();
  ...
  describe('ESLint Auto-Fixing', () => {
    test('should auto-fix common issues', () => {
      ...
      // Act
      const result = execSync(`echo "${testFile}" | bunx eslint --stdin --ext .ts --fix`, {
        cwd: projectRoot,
        stdio: 'pipe',
        encoding: 'utf8',
      });
    ...
    test('should not break code when auto-fixing', () => {
      ...
      expect(() => {
        const result = execSync(`echo "${testFile}" | bunx eslint --stdin --ext .tsx --fix`, {
          cwd: projectRoot,
          stdio: 'pipe',
          encoding: 'utf8',
        });
