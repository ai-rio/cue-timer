...
function setupTestDirectories() {
  return fs
    ...
    .then(() => fs.mkdir(path.join(MOCK_DATA_DIR, 'analytics'), { recursive: true }))
    .then(() => fs.mkdir(path.join(MOCK_DATA_DIR, 'workflow'), { recursive: true }))
    .catch(() => {
...
describe('CLI Integration Tests', () => {
  ...
  describe('blog-create CLI', () => {
    ...
    describe('Blog Creation Functionality', () => {
      test('should create blog post non-interactively with all options', async () => {
        ...
        jest.doMock('../../lib/blog-scripts/content-creator', () => mockContentCreator);
      ...
      test('should support multi-language blog creation', async () => {
        ...
        for (const lang of languages) {
          ...
          const isValidLanguage = supportedLanguages.some((l) => l.value === lang);
          expect(isValidLanguage).toBe(true);

          const result = await mockCreatePost(expect.anything(), expect.anything(), lang);
  ...
  describe('blog-publish CLI', () => {
    ...
    describe('Publishing Workflow', () => {
      test('should validate post before publishing', async () => {
        const mockPost = {
          title: 'Test Post',
          summary: 'A comprehensive test post summary that meets SEO requirements',
          author: 'Test Author',
          tags: ['test', 'blog'],
          readTime: 5,
        };
      ...
      test('should perform SEO check before publishing', async () => {
        ...
        const content =
          'A comprehensive guide about presentation timing with best practices. '.repeat(20);
  ...
  describe('blog-analytics CLI', () => {
    beforeEach(async () => {
      ...
      const analyticsFile = path.join(MOCK_DATA_DIR, 'analytics', 'metrics.json');
    ...
    describe('Analytics Reporting', () => {
      ...
      test('should generate aggregate analytics', async () => {
        const analyticsFile = path.join(MOCK_DATA_DIR, 'analytics', 'metrics.json');
        const analyticsData = JSON.parse(await fs.readFile(analyticsFile, 'utf-8')) as Record<
          string,
        ...
        const avgReadTime = posts.reduce((sum, post) => sum + post.readTime, 0) / posts.length;
  ...
  describe('blog-workflow-status CLI', () => {
    ...
    describe('Workflow Status Tracking', () => {
      test('should show status for specific post', async () => {
        ...
        expect(postStatus.synchronizationStatus.pendingTranslations).toContain('es');
      });

      test('should filter by workflow status', async () => {
        const workflowFile = path.join(MOCK_DATA_DIR, 'workflow', 'status.json');
        const workflowData = JSON.parse(await fs.readFile(workflowFile, 'utf-8')) as Record<
          string,
          { workflowState: string; [key: string]: unknown }
        >;
      ...
      test('should show pending translations', async () => {
        ...
        const postsWithPendingTranslations = Object.entries(workflowData).filter(
          ([_, status]) => (status as any).synchronizationStatus?.pendingTranslations?.length > 0
        );
