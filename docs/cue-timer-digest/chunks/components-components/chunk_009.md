# Chunk 9: components_components

## Metadata

- **Files**: 2
- **Size**: 31,073 characters (~7,768 tokens)
- **Categories**: components

## Files in this chunk

- `components/blog/BlogSearchAndFilterSimple.tsx`
- `components/blog/EnhancedMDXRenderer.tsx`

---

## File: `components/blog/BlogSearchAndFilterSimple.tsx`
...
## File: `components/blog/EnhancedMDXRenderer.tsx`
...
//   }))
// );
const loadCompileMDX = () =>
  import('next-mdx-remote/rsc').then((module) => module.compileMDX);
...
function processEnhancedCustomSyntax(content: string): string {
  ...
  processedContent = processedContent.replace(
    /\{\{tip:([^}]*)\}\}([\s\S]*?)\{\{\/tip\}\}/g,
    (_, level, content) =>
      `<ProTip level="${level || 'beginner'}">${content}</ProTip>`
  );
...
async function EnhancedMDXContent({
  ...
  try {
    ...
    throw error instanceof Error
      ? error
      : new Error('Enhanced MDX compilation failed');
  }
}
