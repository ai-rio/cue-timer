# Chunk 4: components_components

## Metadata

- **Files**: 1
- **Size**: 20,309 characters (~5,077 tokens)
...
## File: `components/blog/AccessibleMDXRenderer.tsx`
    ...
    <a
      ...
      aria-label={
        href?.startsWith('http')
          ? `${String(children)} (opens in new tab)`
          : String(children)
      }
      {...props}
    >
    ...
    <figure className='my-6'>
      <Image
        ...
        aria-describedby={
          alt
            ? undefined
            : `img-desc-${Math.random().toString(36).substr(2, 9)}`
        }
      />
      {alt && (
      ...
      <div className='relative rounded-lg border bg-gray-900 overflow-hidden'>
        ...
        >
          <div className='flex items-center gap-2'>
            <span
              className='font-mono'
              aria-label={`File: ${filename || language}`}
            >
              {filename || language}
            </span>
            {allowRun && (
          ...
          >
            {allowRun && (
              <Button
                ...
                aria-describedby={
                  isRunning ? undefined : `${blockId}-run-status`
                }
              >
                {isRunning ? (
                  <>
...
export function AccessibleCallout({
  ...
  const defaultIcon =
    typeIcons[type as keyof typeof typeIcons] || typeIcons.info;
            ...
            fallback={
              fallback || (
                <div
                  className='space-y-4'
                  role='status'
                  aria-label='Loading enhanced content'
                >
        ...
        <div className='space-y-4'>
          ...
          >
            <p className='text-sm text-blue-800'>
              ...
              components, syntax highlighting, or custom elements. Click to
              button above to load the full accessible experience.
            </p>
          </div>
        </div>
      </main>
