# Chunk 89: source_scripts

## Metadata

- **Files**: 1
- **Size**: 38,871 characters (~9,717 tokens)
- **Categories**: source
...
## File: `scripts/system-health-check.ts`
...
class SystemHealthChecker {
  ...
  async runHealthCheck(): Promise<SystemHealthReport> {
    console.log(
      chalk.bold.blue('🏥 CueTimer Blog Management System - Health Check')
    );
    ...
    try {
      ...
      if (nodeModulesExists) {
        try {
          ...
          const duOutput = execSync('du -sb node_modules 2>/dev/null', {
            encoding: 'utf8',
          });
      ...
      let outdatedCount = 0;
      try {
        const outdatedOutput = execSync('bunx npm-check-updates 2>/dev/null', {
          encoding: 'utf8',
        });
      ...
      try {
        const auditOutput = execSync('bunx audit --json 2>/dev/null', {
          encoding: 'utf8',
        });
        if (auditOutput) {
      ...
      if (hasCriticalIssues) {
        ...
        if (!nodeModulesExists)
          this.alerts.push('node_modules directory missing');
        if (criticalDepsMissing.length > 0) {
          this.alerts.push(
            `Missing critical dependencies: ${criticalDepsMissing.join(', ')}`
          );
        }
      } else if (hasWarnings) {
        status = 'warning';
        ...
        if (vulnerabilities.total > 0) {
          ...
          this.recommendations.push(
            'Run `bun audit fix` to resolve vulnerabilities'
          );
        }
      } else {
        status = 'healthy';
  ...
  private async checkEnvironment(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      const nodeMajor = nodeVersionMatch
        ? parseInt(nodeVersionMatch[1] || '0')
        : 0;
      ...
      if (!metrics.nodeVersionSupported) {
        this.alerts.push(
          `Node.js version ${nodeVersion} is not supported (requires >= 18.0.0)`
        );
      }
      ...
      if (existsSync('.env.example')) {
        ...
        const documentedVars = requiredVars.filter((v) =>
      ...
      if (gitignoreExists) {
        ...
        gitignoreProtectsSensitive =
          protectedPatterns.length === sensitivePatterns.length;

        if (!gitignoreProtectsSensitive) {
          this.alerts.push(
            'Sensitive files may not be properly protected in .gitignore'
          );
        }
      } else {
      ...
      // Determine health status
      const hasCriticalIssues =
        !metrics.nodeVersionSupported || !gitignoreExists;
  ...
  private async checkFileSystem(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      if (missingDirs.length > 0) {
        this.alerts.push(
          `Missing critical directories: ${missingDirs.join(', ')}`
        );
      }
      ...
      if (permissionIssues.length > 0) {
        this.alerts.push(
          `Permission issues with files: ${permissionIssues.join(', ')}`
        );
      }
  ...
  private async checkCliTools(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      if (inaccessibleTools.length > 0) {
        this.alerts.push(
          `${inaccessibleTools.length} CLI tools are not accessible`
        );
        this.recommendations.push(
          'Check script files and package.json configuration'
        );
      }
  ...
  private async checkTemplateSystem(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      const templatesComplete =
        existingTemplates.length === expectedTemplates.length;
  ...
  private async checkMultiLanguageSupport(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      if (existsSync(i18nConfig)) {
        ...
        i18nConfigured =
          config.includes('i18n') || config.includes('next-intl');
      }
      ...
      if (localesDirExists) {
        try {
          const findOutput = execSync(
            `find ${localesDir} -name "*.json" 2>/dev/null`,
            {
              encoding: 'utf8',
            }
          );
      ...
      if (missingLocales.length > 0) {
        this.recommendations.push(
          `Add missing locale files: ${missingLocales.join(', ')}`
        );
      }
      ...
      const hasI18nSupport =
        i18nConfigured && localesDirExists && localeFiles.length > 0;
      ...
      if (!hasI18nSupport) {
        ...
        this.recommendations.push(
          'Configure internationalization for blog system'
        );
  ...
  private async checkContentSystem(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      if (existsSync('content')) {
        try {
          const findOutput = execSync(
            'find content -name "*.mdx" 2>/dev/null',
            {
              encoding: 'utf8',
            }
          );
      ...
      if (invalidMdxFiles.length > 0) {
        this.alerts.push(
          `${invalidMdxFiles.length} MDX files have formatting issues`
        );
      }
      ...
      const contentValidationExists = existsSync(
        'lib/blog-scripts/content-validator.ts'
      );
      ...
      const seoOptimizationExists = existsSync(
        'lib/blog-scripts/seo-optimizer.ts'
      );
      ...
      // Determine health status
      const contentStructureValid =
        dirStatus['content'] && dirStatus['content/blog'];
      ...
      if (!contentStructureValid || !contentCreatorExists) {
        ...
        !noInvalidContent ||
        !contentValidationExists ||
        !seoOptimizationExists
      ) {
        status = 'warning';
  ...
  private async checkPerformance(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      const performanceGood =
        canStart && (!bundleSize || bundleSize < 100 * 1024 * 1024);
      ...
      return {
        ...
        details: {
          ...
          bundleSize: bundleSize
            ? `${Math.round(bundleSize / 1024 / 1024)}MB`
            : 'unknown',
  ...
  private generateSummary(
    ...
    return {
      ...
      criticalComponents: statuses.filter((s) => s.status === 'critical')
        .length,
  ...
  private displayHealthReport(report: SystemHealthReport): void {
    ...
    if (report.overall.status === 'healthy') {
      console.log(
        chalk.green.bold('🎉 System is healthy and ready for production!')
      );
    } else if (report.overall.status === 'warning') {
      console.log(
        chalk.yellow.bold('⚠️  System has warnings that should be addressed.')
      );
    } else {
      console.log(
        chalk.red.bold('🚨 System has critical issues that must be resolved.')
      );
    }
  }
