...
- **Files**: 1
- **Size**: 23,862 characters (~5,965 tokens)
- **Categories**: tests
...
async function createMockBlogPost(
  slug: string,
    ...
    if (slug) {
      ...
      if (!postAnalytics) {
        return {
          success: false,
          output: '',
          error: `No analytics data for post: ${slug}`,
        };
      }
    ...
    if (slug) {
      ...
      if (!postStatus) {
        return {
          success: false,
          output: '',
          error: `No workflow data for post: ${slug}`,
        };
      }
...
describe('CLI Integration Tests', () => {
  ...
  describe('blog-create CLI Integration', () => {
    ...
    test('should validate required parameters', async () => {
      ...
      const result1 = await simulateCliCommand('blog-create', [
        { title: 'Test post' },
      ]);
      ...
      // Test missing title
      const result2 = await simulateCliCommand('blog-create', [
  ...
  describe('End-to-End CLI Workflow Tests', () => {
    test('should handle complete blog creation to analytics workflow', async () => {
      ...
      const createResult = await simulateCliCommand('blog-create', [
        createOptions,
      ]);
      ...
      const analyticsResult = await simulateCliCommand('blog-analytics', [
        { slug },
      ]);
      ...
      const workflowResult = await simulateCliCommand('blog-workflow-status', [
        { slug },
      ]);
      ...
      // Step 7: Publish post
      const publishResult = await simulateCliCommand('blog-publish', [
        { slug, dryRun: true },
      ]);
    ...
    test('should handle error scenarios gracefully', async () => {
      ...
      const invalidAnalyticsResult = await simulateCliCommand(
        'blog-analytics',
        [
          {
            slug: 'non-existent',
      ...
      const invalidWorkflowResult = await simulateCliCommand(
