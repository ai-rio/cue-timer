...
- **Files**: 1
- **Size**: 14,722 characters (~3,680 tokens)
- **Categories**: tests
...
## File: `tests/scripts/blog-create.test.ts`
...
describe('blog-create CLI Script', () => {
  ...
  describe('Template Validation', () => {
    ...
    test('should throw error for invalid template in non-interactive mode', async () => {
      ...
      // Act & Assert
      await expect(createBlogPost(options)).rejects.toThrow(
  ...
  describe('Error Handling', () => {
    test('should handle ContentCreator errors gracefully', async () => {
      ...
      const MockContentCreator = ContentCreator as jest.MockedClass<
      ...
      MockContentCreator.mockImplementation(
        () =>
          ({
            createPost: jest
              .fn()
              .mockRejectedValue(new Error('Content creation failed') as never),
          }) as any
      );
