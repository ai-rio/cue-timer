...
- **Files**: 1
- **Size**: 20,406 characters (~5,101 tokens)
- **Categories**: tests

## Files in this chunk

- `tests/integration/mdx-renderers.test.tsx`

---

## File: `tests/integration/mdx-renderers.test.tsx`
...
`;
  ...
  describe('AccessibleMDXRenderer', () => {
    test('should render basic markdown content with proper accessibility', async () => {
      ...
      domMatchers(
        screen.getByText('This is a paragraph with')
      ).toBeInTheDocument() as any;
      domMatchers(
...
`;
      ...
      domMatchers(
        screen.getByLabelText('File: javascript')
      ).toBeInTheDocument() as any;
    });
...
`;
    ...
    test('should provide proper ARIA labels for external links', () => {
      // Arrange & Act
      render(
        <AccessibleMDXRenderer content='[External](https://example.com)' />
      );
...
`;
    ...
    test('should load enhanced features when requested', async () => {
      ...
      // Assert
      domMatchers(
        screen.getByText(/Loading enhanced features.../i)
      ).toBeInTheDocument() as any;
    });
  });
  ...
  describe('EnhancedMDXRenderer', () => {
    test('should render enhanced content with template components', async () => {
      ...
      await waitFor(() => {
        domMatchers(
          screen.getByTestId('compiled-mdx')
        ).toBeInTheDocument() as any;
      });
      expect(mockReadingTimeUpdate).toHaveBeenCalled();
...
`;
      ...
      domMatchers(
        screen.getByText("console.log('simple');")
      ).toBeInTheDocument() as any;
    });
  });
...
`;
  ...
  describe('ESLint Compliance Tests', () => {
    ...
    test('should use proper React patterns', () => {
      ...
      domMatchers(
        screen.getByText('React Patterns Test')
      ).toBeInTheDocument() as any;
  ...
  describe('Performance Optimization', () => {
    test('should use lazy loading for heavy dependencies', () => {
      ...
      domMatchers(screen.getByRole('main')).toBeInTheDocument() as any;
      (
        expect(screen.queryByTestId('syntax-highlighter')) as any
    ...
    test('should only load enhanced features when needed', () => {
      ...
      const { rerender } = render(
        <OptimizedMDXRenderer content={simpleContent} />
      );
      (
        expect(screen.queryByText(/advanced features/i)) as any
      ).not.toBeInTheDocument();

      // Rerender with complex content
      rerender(<OptimizedMDXRenderer content={complexContent} />);
      domMatchers(
        screen.getByText(/advanced features/i)
      ).toBeInTheDocument() as any;
    });
...
`;
  ...
  describe('Cross-Component Integration', () => {
    test('should handle content transitions between renderers', async () => {
      ...
      const user = userEvent.setup();
      const { rerender } = render(
        <OptimizedMDXRenderer content={basicContent} />
      );
...
`;
      ...
      const { rerender } = render(
        <AccessibleMDXRenderer content={accessibleContent} />
      );
      domMatchers(screen.getByRole('main')).toBeInTheDocument() as any;
      domMatchers(screen.getByRole('list')).toBeInTheDocument() as any;
