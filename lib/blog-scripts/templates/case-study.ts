...
export function generateCaseStudyContent(variables: CaseStudyVariables): string {
  const {
    title,
    client,
    industry,
  ...
  const defaultImplementation = [
    ...
    },
    {
      ...
      description: 'Hands-on training sessions for speakers, moderators, and event coordinators',
      timeline: 'Week 4',
    },
    {
      step: 'Pilot Implementation',
      description: 'Gradual rollout with initial presentations and feedback collection',
      timeline: 'Week 5-6',
    },
    {
      step: 'Optimization & Scale',
      description: 'Fine-tuning based on feedback and expanding to all presentation events',
      timeline: 'Week 7-8',
    },
  ];
...
export function createCaseStudyVariables(
  ...
  const defaultIndustry =
    customVariables.industry || industries[Math.floor(Math.random() * industries.length)];
  const defaultClient = customVariables.client || 'Global Solutions Inc.';

  return {
    title: customVariables.title || `How ${defaultClient} Transformed Presentations with CueTimer`,
    ...
    results: customVariables.results || [
      ...
      },
      {
        ...
        value: '+67%',
        description: 'Measurable increase in audience participation and attention',
      },
      {
        metric: 'On-Time Completion',
        value: '95%',
        description: 'Presentations now consistently finish within scheduled time slots',
      },
      {
